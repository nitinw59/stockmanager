...
<script>
		$(document).ready(function(){
			$(".updateFit").click(function(){
		  	window.location.replace("updateFit.php?fitType="+$(this).val());
        });
		 



      $(".deleteFit").click(function(){
...
<?php
    ...
    $querry="select * from fitTypes";

    if($result=mysqli_query($dbhandle,$querry)){
        while($row=mysqli_fetch_array($result)){
            $a= $row[0];
            $b= $row[1];
            echo "<tr  id='$a'><td >  $a  </td> <td>$b</td> <td><center><button type='submit' class='updateFit' value=$a>UPDATE</button><center></td><td><center><button type='submit' class='deleteFit' value=$a>Delete</button><center></td></tr>";
            echo "";
